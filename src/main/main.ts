...
// 单一事实来源:所有配置路径都从 configDir 派生;core 层 ai-config 亦对齐(见 ai-extract.getConfigPath)。
const configDir = path.join(dataRoot, 'config');

// 浏览器登录态复用配置 + 默认 profile 目录(profile 是浏览器状态目录,非配置文件,仍留 dataRoot 根)
const browserConfigPath = path.join(configDir, 'browser-config.json');
const defaultProfileDir = path.join(dataRoot, 'browser-profile');

// 录制端请求改写规则:文件路径(默认 enabled=false,不干预录制)
const requestRulesPath = path.join(configDir, 'request-rules.json');

// 回放行为档:文件路径(首次生成含 default/slow-site/anti-bot 三档;default=现状写死值)
const replayProfilePath = path.join(configDir, 'replay-profile.json');
...
function ensureDirs(): void {
    for (const dir of [macrosDir, exportsDir, errorsDir, downloadsDir, timelinesDir, dumpsDir, configDir]) {
