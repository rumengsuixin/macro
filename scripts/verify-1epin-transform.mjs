...
// 货币符号:两条线的金额字段都在链首全局去 $/＄。符号可能前置、后置或与数字间带空格
// ——带空格那种若不先去符号,会被后面「剥币种后缀」那步(\s[\s\S]*$)截成只剩符号/空串。
console.log('货币符号:金额字段去 $/＄(半角+全角,不伤小数)');
check('订单线 单价 符号前置', 单价, '$1.065 USD', '1.065');
check('订单线 单价 符号后置', 单价, '1.065 $', '1.065');
check('订单线 单价 符号后带空格', 单价, '$ 1.065 USD', '1.065');
check('订单线 全角＄', 单价, '＄1.065 USD', '1.065');
check('订单线 负号金额保号', 单价, '-$5.00 USD', '-5.00');
check('订单线 高精度余额带符号仍不截断(铁律)', fieldOf(orders, '交易前余额(USD)'), '$2875.1723556 USD', '2875.1723556');
check('付款线 金额符号前置(逗号小数保真)', 付款金额, '$3000,00 USD / Kyle KK\n($3000,000USDT)', '3000,00');
check('付款线 金额符号后带空格', 付款金额, '$ 3000,00 USD / Kyle KK', '3000,00');
check('付款线 加密金额剥括号内符号', 加密金额, '$3000,00 USD / Kyle KK\n($3000,000USDT)', '3000,000USDT');
check('付款线 前余额带符号', 前余额, 'Önce: $106,60 USD\nSonra: $3106,60 USD', '106,60');
check('付款线 后余额带符号', 后余额, 'Önce: $106,60 USD\nSonra: $3106,60 USD', '3106,60');
check('付款线 后余额单行带符号仍为空(不误抄)', 后余额, 'Önce: $106,60 USD', '');
