...
- 选择器质量加固(通用,非专项):①`ai-extract.ts` 的 `SELECTOR_QUALITY_GUIDE` 常量(优先 data-*/id/aria/文本、禁用框架动态类名如 `el-table_<n>_column_<n>`/Vue `v-xxxx`/CSS-Modules 哈希、慎用结构伪类、注意组件库固定列/虚拟滚动的克隆 DOM 选可见可交互那份、actionSelector 须项内可点)在 `generateExtract` 组装 message 时**无条件注入**(systemPrompt 后、modeHint 前),不依赖既有 ai-config.json,老装机即时生效。②**生成后自检回路**:`GenerateInput` 加 `feedback`/`sessionKey`,`GenerateResult` 回传 sessionKey(`chat.send` 仅凭 sessionKey 路由 agent,故重试须复用首轮 key 才能保留上下文);renderer 的 `verifySelectors` 在录制 webview 内用一段 `executeJavaScript` 实测各选择器命中数(listSelector 整页测,字段/actionSelector/detailLinkSelector 以**首个列表项**为根测,镜像 extractor 的 `item.locator(sel).first()`;非法选择器记 invalid),`evaluateChecks` 判定(listSelector 必>0、非法即失败、项内至少一个命中),未通过则带中文反馈复用同会话重生成,**最多 2 次修复**,仍失败不致命(填最后一版+提示人工核对)。DOM 与 AI 所见一致,无需回放浏览器、无新 IPC。
