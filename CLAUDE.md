...
- **核心自检**:`verify-core.mjs`(跑 demo 宏导出 Excel)· `verify-pagination.mjs`(翻页 3 页 30 行,需网络)· `verify-merge.mjs`(合并后处理器)· `verify-captures.mjs`(旁车往返)· `verify-cancel.mjs`(运行中停止)· `verify-popup-wait.mjs`(新窗口等待竞态)· `verify-stepurls.mjs`(回放回填来源 URL)· `verify-selector-class.mjs`(稳定类判定)· `verify-list-action-multi.mjs`(list-action 多动作 + scope 点击序列)· `verify-list-action-filter.mjs`(list-action 行级筛选:text/AND/OR/exists/attr/数值/失败即安全)· `verify-list-action-action-filter.mjs`(list-action 动作级 gate + 收尾 finally:弹窗内取值过滤 / abort·skip 差异 / finally 每行清场)· `verify-bank-integrate.mjs`(银行整合 5 代号真跑 exe)
