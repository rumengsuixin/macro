...
- **运行时配置**(已 gitignore,首次运行自动生成):`ai-config.json`(AI profile)· `browser-config.json`(登录态复用/反检测)· `request-rules.json`(拦截 8 支路)· `bank-integrate.json`(银行整合 exe 路径)· `merge-config.json`(合并解析规则:按文件名/工作表名匹配,指定表头行/裁列/工作表)· `<宏名>.captures.json`(选择器上下文旁车)
...
- **核心自检**:`verify-core.mjs`(跑 demo 宏导出 Excel)· `verify-pagination.mjs`(翻页 3 页 30 行,需网络)· `verify-merge.mjs`(合并后处理器)· `verify-captures.mjs`(旁车往返)· `verify-cancel.mjs`(运行中停止)· `verify-popup-wait.mjs`(新窗口等待竞态)· `verify-stepurls.mjs`(回放回填来源 URL)· `verify-selector-class.mjs`(稳定类判定)· `verify-list-action-multi.mjs`(list-action 多动作 + scope 点击序列)· `verify-list-action-filter.mjs`(list-action 行级筛选:text/AND/OR/exists/attr/数值/失败即安全)· `verify-list-action-action-filter.mjs`(list-action 动作级 gate + 收尾 finally:弹窗内取值过滤 / abort·skip 差异 / finally 每行清场)· `verify-merge-config.mjs`(合并配置驱动:默认乱表 / matchSheet·match 命中裁出干净表 / 首次生成模板)· `verify-bank-integrate.mjs`(银行整合 5 代号真跑 exe)
...
- **后处理器**:轻量注册表 `post-processors/index.ts`(`type→{manifest,handler}`),新 handler 自注册 + `index.ts` 末尾 `import` 即热插拔上架。首个 `merge-zip-excel`(解压 zip / 裸文件 → SheetJS 读 csv/xls/xlsx、UTF-8+GBK 兜底 → exceljs 堆叠合并;解析**配置驱动** `merge-config.json`——按文件名 glob 或工作表名 glob 匹配规则,指定表头行 `headerRow`/裁列 `endColumn`/工作表 `sheet`,适配带标题块的模板;无配/无匹配 = 历史行为)。
