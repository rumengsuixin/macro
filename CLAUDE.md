...
- **提取 4 模式**(`ExtractConfig.mode`):`single`(整页单行)· `list`(逐项采字段)· `list-detail`(逐项进详情页采字段)· `list-action`(逐项按序执行动作、常用于点按钮下载,不采数据;`actionSelector` 支持单字符串或**多动作数组**,每动作带 `scope` 项内/全局;另支持**行级筛选** `filter`——仅匹配条件的行才执行动作:内置变量 `text`/`html`(本行文本/HTML)+ 可选命名变量 `vars`,`conditions` 用 when 表达式引擎判定、多条按 `match` 切 AND/OR,失败即安全;侧栏 AI 面板有随模式显隐的**专属可视化配置区**——多行动作/筛选条件/变量增删 + 🎯点选,双向同步 JSON)。翻页由步骤右键「标记翻页操作」+ 总页数控制(标了才逐页处理)。
...
- **核心自检**:`verify-core.mjs`(跑 demo 宏导出 Excel)· `verify-pagination.mjs`(翻页 3 页 30 行,需网络)· `verify-merge.mjs`(合并后处理器)· `verify-captures.mjs`(旁车往返)· `verify-cancel.mjs`(运行中停止)· `verify-popup-wait.mjs`(新窗口等待竞态)· `verify-stepurls.mjs`(回放回填来源 URL)· `verify-selector-class.mjs`(稳定类判定)· `verify-list-action-multi.mjs`(list-action 多动作 + scope 点击序列)· `verify-list-action-filter.mjs`(list-action 行级筛选:text/AND/OR/exists/attr/数值/失败即安全)· `verify-bank-integrate.mjs`(银行整合 5 代号真跑 exe)
